  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-800 via-green-600 to-slate-900 font-sans">
          ...
          <div className="container mx-auto px-4 text-center">
            ...
            <div className="space-y-6">
              ...
              >
                <Button
                  size="lg"
                  className="bg-gradient-to-r cursor-pointer animate-pulse from-green-600 to-orange-600 hover:from-sky-600 hover:to-orange-700 text-white text-sm md:text-2xl px-6 py-4 md:px-12 md:py-8 rounded-full shadow-2xl transform hover:scale-105 transition-all duration-300 drop-shadow-xl w-full sm:w-auto max-w-full whitespace-normal break-words"
                >
                  <Users className="mr-3 h-6 w-6 md:h-8 md:w-8" />
      ...
      <Link
        ...
        target="_blank"
        rel="noopener noreferrer"
  aria-label="Abrir WhatsApp (nova aba)"
  className="fixed bottom-6 right-6 z-50 bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-2xl transform hover:scale-110 transition-all duration-300 group"
      >
        {/* Visually-hidden label for screen readers */}
        <span className="sr-only">Abrir WhatsApp</span>

        {/* Pulsing ring (non-interactive) */}
        <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <span className="w-8 h-8 md:w-9 md:h-9 bg-green-400 rounded-full opacity-40 animate-ping"></span>
        </span>

        {/* Inline SVG for WhatsApp icon (kept same path) */}
        <svg
          className="relative z-10 w-6 h-6 md:w-7 md:h-7 text-white transition-transform duration-200 group-hover:scale-110"
          fill="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
